...
inline std::u16string ReadNullTermString(const char16_t* ptr) {
...
struct TitlePlayed {
  ...
  void ReadGPD(const X_XDBF_GPD_TITLEPLAYED* src) {
    ...
    title_name = ReadNullTermString((const char16_t*)txt_ptr);
  }
  ...
  void WriteGPD(X_XDBF_GPD_TITLEPLAYED* dest) {
    ...
    auto* txt_ptr = reinterpret_cast<const uint16_t*>(dest + 1);
    xe::copy_and_swap((char16_t*)txt_ptr, (char16_t*)title_name.c_str(),
      title_name.size());
  }
};
...
struct Achievement {
  ...
  void ReadGPD(const X_XDBF_GPD_ACHIEVEMENT* src) {
    ...
    label = ReadNullTermString((const char16_t*)txt_ptr);

    txt_ptr += (label.length() * 2) + 2;
    description = ReadNullTermString((const char16_t*)txt_ptr);

    txt_ptr += (description.length() * 2) + 2;
    unachieved_desc = ReadNullTermString((const char16_t*)txt_ptr);
  }
};
