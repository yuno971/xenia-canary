...
UserProfile::UserProfile() : dash_gpd_(kDashboardID) {
  ...
  account_.xuid_online = 0xB13EBABEBABEBABE;

  // TODO: How to deal with this nullterm?
  auto default_user_name = u"XeniaUser\0\0";

  std::memcpy(account_.gamertag, default_user_name,
              (std::u16string(default_user_name).size() * 2) + 2);
...
bool UserProfile::UpdateGpd(uint32_t title_id, xdbf::GpdFile& gpd_data) {
  ...
  filesystem::CreateFile(cvars::profile_directory / fname);
  auto mmap_ =
