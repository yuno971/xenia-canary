...
bool ContentPackage::Mount(std::string root_name,
  ...
  if (!fs->RegisterSymbolicLink(root_name + ':', device_path_.string())) {
    return false;
  }
...
bool ContentPackage::Unmount() {
  ...
  root_names_.clear();
  fs->UnregisterDevice(device_path_.string());
...
bool FolderContentPackage::Mount(std::string root_name) {
  ...
  if (!device_inited_) {
    device = std::make_unique<vfs::HostPathDevice>(device_path_.string(), package_path_.string(),
                                                   false);
...
X_RESULT FolderContentPackage::GetThumbnail(std::vector<uint8_t>* buffer) {
  auto thumb_path = package_path_ / kThumbnailFileName;
...
X_RESULT FolderContentPackage::SetThumbnail(std::vector<uint8_t> buffer) {
  ...
  filesystem::CreateParentFolder(package_path_);
...
bool StfsContentPackage::Mount(std::string root_name) {
  ...
  if (!device_inited_) {
    device =
        std::make_unique<vfs::StfsContainerDevice>(device_path_.string(), package_path_.string());
    if (!device->Initialize()) {
      return false;
    }
