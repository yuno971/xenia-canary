...
X_STATUS UserModule::LoadFromFile(const std::string_view path) {
  ...
  if (cvars::xex_apply_patches) {
    ...
    if (!file_system->FindSymbolicLink("update:", link_path)) {
    ...
    // First try checking update:\ root for patch, otherwise try same path as
    // XEX
    auto patch_entry =
        file_system->ResolvePath("update:\\" + module_path + "p");
    if (!patch_entry) {
