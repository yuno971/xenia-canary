...
DEFINE_bool(clock_no_scaling, false,
            "Disable scaling code. Time managemend and locking is bypassed. "
...
uint64_t UpdateGuestClock() {
  if (cvars::clock_no_scaling) {
    ...
    auto host_tick_count = Clock::QueryHostTickCount();
...
inline uint64_t QueryGuestSystemTimeOffset() {
  ...
  auto numerator = 10000000ull;  // 100ns/10MHz resolution
  auto denominator = guest_tick_frequency_;
...
uint32_t Clock::QueryGuestUptimeMillis() {
  return static_cast<uint32_t>(std::min(QueryGuestSystemTimeOffset() / 10000,
...
uint32_t Clock::ScaleGuestDurationMillis(uint32_t guest_ms) {
  ...
  return uint32_t(std::min(scaled_ms, static_cast<uint64_t>(UINT_MAX)));
}
