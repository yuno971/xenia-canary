...
bool EmulatorWindow::Initialize() {
  ...
  // Recent Sub tab
  auto recent_list = MenuItem::Create(MenuItem::Type::kPopup, "&Open Recent");
  ...
  if (recent_file_read.is_open()) {
    while ((i < 10) && getline(recent_file_read, lines[i])) {
      ...
      global_recent_paths_[i] = xe::to_path(lines[i]);
      // Add most recently opened files to menu
      if (lines[i] != "" && lines[i] != "\n") {
        recent_list->AddChild(
            MenuItem::Create(MenuItem::Type::kString, global_recent_paths_[i].u8string(),
...
void EmulatorWindow::RecentListUpdater(const std::filesystem::path& path) {
  //*******Save abs_path to recent as UTF-8******************************
  std::string u8_path = path.u8string();
